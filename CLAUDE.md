...
```bat
start_app.bat           # one-click: setup if needed + both servers (no reload/debug) + browser
stop_app.bat            # kill both servers (window titles, then ports 8000/8050)
setup_venv.bat          # one-time: create .venv and install deps (torch from the CPU wheel index)
run_dev.bat             # dev terminal 1: FastAPI on :8000 (auto-reload)
run_frontend.bat        # dev terminal 2: Dash UI on :8050 (APP_DEBUG=1 → werkzeug debug)
```
...
**Skip-cleaning mode (2026-07-15):** `CleanRequest.skip_cleaning` → `clean_task(skip_cleaning=)` → `cleaning_agent.run(skip=True)` writes a pass-through recipe (`recipe_source: "skipped"`, no LLM call) and applies `_apply_essentials` INSTEAD of `_sanitize_recipe`: only inferred-dtype casts, timestamp `parse_datetime` + `drop_row`-on-null, and the sort — no fills, no outlier treatment, no column drops. For data that's already clean or when cleaning causes more trouble than it prevents. UI: the `switch-skip-clean` toggle on the Setup intent form (also read by the Cleaning card's re-run button; `None` when unmounted → False). Like `use_llm`, it's a transient flag — excluded from `forecast_user_selections.json`.
...
**Non-finite stats must never reach stage JSON (2026-07-15):** pandas returns NaN for undefined moments (kurtosis needs ≥4 points, skew ≥3, std ≥2 — low-cardinality columns hit this routinely), `json.dump` happily writes a literal `NaN` token, `json.load` reads it back — but FastAPI/Starlette's response serializer uses `allow_nan=False`, so a single NaN anywhere in `metadata.json` 500'd `/summary` and `/metadata` and made the run unloadable (found on the 5.3M-row MCC grievances ingest). Fixes: `ingest.py::_finite()` maps non-finite → None at both numeric_stats build sites, and `routes.py::_read_stage_json` passes `parse_constant=lambda _: None` so runs with NaN already on disk still load. Consumers are None-tolerant: `pre_clean_eda._constant_cols`/column profile skew, `validation_gate` numeric_variance, frontend `_fmt` (renders "—"). Keep both layers — any new stage writer with float stats needs the same treatment.
...
| `/runs/{id}/forecast-eda` | POST | Stage 4 — forecast EDA from confirmed intent (body = optional `scope`/`forecast_frequency`/`horizon`/`exog_cols` overrides — none re-clean; per-series without a confirmed series key → 400; exog cols not in the cleaned data → 400; plus the **training window** `train_start`/`train_end`/`exclude_ranges` — for these three, presence in the body is authoritative via `model_fields_set`, null = clear back to full data; inverted bounds → 400), job-managed |
...
| `/runs/{id}/train-estimate` | GET | Wall-clock training cost estimate (`?models=a,b&profile=`) via `trainer.estimate_for_run` — same numbers as the `/train` heavy gate; feeds the Training card's live banner. Lightweight, NOT job-managed |
| `/runs/{id}/progress` | GET | Live pipeline progress: `runs/{id}/progress.json` (stages/log/per-model train events, written by `pipeline/progress.py` from inside the job child) + `job_active` flag (`jobs.active_track_id()`). Polled every 1.5s by the UI while a chain callback is in flight. Lightweight, NOT job-managed |
...
progress.json               # live per-stage progress + log ring (pipeline/progress.py;
                            #   rewritten continuously; read by GET /runs/{id}/progress)
```
...
1. Drop columns (`action = "drop"`) — pure exclusion from the output, nothing loaded
2. Type fixes (`type_fix`) — applied per column AT LOAD (see columnar note below)
3. **Sort by (series, time)** — `recipe.group_cols` (from the confirmed intent) + timestamp — **only when an order-sensitive strategy exists** (`_needs_time_order`: rolling_iqr/stl_residuals or interpolate/ffill/bfill/flag_and_fill). Distribution and groupby-transform strategies don't need row order, and skip-cleaning recipes have no strategies at all — those keep raw order until step 8, saving a full-frame sort copy
...
7. Drop duplicates — full-row semantics via combined per-column 64-bit hashes (`_dedupe_keep_mask`), so passive columns participate without being materialized
...
**Columnar execution (2026-07-16):** the cleaner no longer loads the whole frame into pandas. Only **active** columns (recipe touches their values, or the executor reads them: timestamp / series key / target) are materialized — one at a time, type-fixed at load so casts store 8-byte values instead of Python strings. **Passive** columns stay in Arrow and are row-aligned to the output with one `take(orig)` each at write time; `orig` is the row-index array threaded through sort/drops/dedupe. Rationale: the old whole-frame load was ~8 GB in pandas for a 5.3M×58 dataset (> the 7.8 GB box) and a "skip cleaning" run swap-thrashed for 21 minutes — skip mode was the WORST case precisely because it drops no columns. Measured after: skip 1264s → 92s (peak 4.2 GB), fallback recipe 411s → 78s (peak 3.9 GB); reports and outputs verified identical vs the old cleaner (synthetic all-strategy battery + the real MCC run; gate passes). One deliberate order difference: rows tied on the same timestamp keep raw order instead of series order when no order-sensitive strategy ran — nothing downstream reads tie order (Stage 4/6 resample immediately).

**`_parse_datetime_fast` (cleaner.py):** all cleaner datetime parsing goes through a layered format parser — candidate formats guessed from a spread sample (plus bare-time `%H:%M:%S`/`%H:%M`, which pandas' guesser never returns), each strict vectorized parse claims what it matches (majority format first), only the residue pays per-element dateutil. Sample-validated against the lenient path (must parse ≥ as many values) or it falls back wholesale. Why: format-less `pd.to_datetime` on the mixed-format `RECVDTIME` ('17:24' AND '19:38:42' in one column) cost 94s alone; layered = 11s. Bare-time values now get the deterministic 1900-01-01 base date instead of dateutil's "today" (better: re-runs no longer shift dates; intent detection demotes time-only columns anyway).

**Per-series execution:** when `recipe.group_cols` is non-empty, every series-boundary-sensitive operation runs WITHIN each series via groupby — **always `dropna=False` (2026-07-15)**: pandas' groupby default EXCLUDES rows whose key is null, and the index-aligned assignment back then writes NaN into every excluded row. Observed real failure: WARDNAME (99.94% null) as series key turned per-series forward_fill into a mass erase of four columns' values (6% null → 99.94%, `no_null_regression` gate failure). Null-key rows form their own "(unknown)" series. Any NEW per-series groupby in cleaner.py must carry `dropna=False`: `rolling_iqr`, `stl_residuals`, `interpolate`, `forward_fill`/`backward_fill`, `flag_and_fill`, `mean_fill`/`median_fill` (per-series statistic), and the `remove` strategy's IQR fences (per-series quantile transform). A forward-fill must never carry one society's last value into the next society's first row, and a small series' normal values are outliers only relative to ITS OWN distribution. Global-distribution strategies (`clip_iqr`, `winsorize`, `log_transform`) deliberately stay global. Per-series STL is gated at `_STL_MAX_GROUPS` (20) — past that many series the O(n) classical decomposition runs inside every series instead (statsmodels STL × 1,800 series would take tens of minutes).
...
**Execution-time target-level guard (cleaner.py, 2026-07-15):** the sanitizer's "level-neutral" allowance for `rolling_iqr`/`stl_residuals` on the target is an assumption, and it broke: on the MCC grievances target (`NOOFCOMPLDAYS`, ±700k junk sentinels, raw total NEGATIVE) rolling_iqr moved the total +182% and BOTH recipe sources hard-failed the gate — a dead end for the user. So the cleaner now VERIFIES: around any value-mutating outlier strategy on a sum/mean target it measures the column total, and if the drift alone would breach `validation_gate.max_target_sum_drift_pct` (read from settings via `_target_drift_threshold()` so guard and gate can never disagree) it reverts the column and records `target_outlier_reverted` `{column, strategy, would_be_drift_pct, threshold_pct}` in the report/return — the run proceeds with the raw target, the gate passes, and the Cleaning card shows a "Target protected" warning instead of a stuck failure.
...
- **Timestamps** ranked business-vs-audit (`created/loaded/inserted…` names lose); multiple business timestamps (vet data has 3 event times) → low confidence, the user picks. **Time-of-day-only columns (2026-07-15)** — split date/time DB schemas store bare times (`RECVDTIME='11:05:33'` beside `RECVDDATE`); pandas stamps them with TODAY's date so the parsed span collapses to one day, and the granularity ranking used to crown them (mcc: 5.3M rows → "1 distinct daily period", gate dead-end). `_is_time_only()` (sampled parse → `nunique(normalize) <= 1`) demotes them below everything and the intent dropdown labels them "⚠ time-of-day only, no date".
...
**Training window (2026-07-16):** `forecast_user_selections.json` optionally carries `train_start`/`train_end`/`exclude_ranges` (set from the Forecast EDA tab's collapsed "Advanced: training window" section — `date-fcst-train-range` DatePickerRange, exclusions added via a picker+Add button into the multi-dropdown `dropdown-fcst-exclude` whose tag × removes entries, values encoded `"start..end"`). `forecasting_eda.apply_train_filter(df, ts_col, sel)` applies it (range bounds inclusive of whole days; exclusions drop rows → empty resample bins → the standard fill rules) at BOTH load sites: `forecasting_eda.run()` and `feature_builder.run()` (which imports it) — the trainer inherits via the model frame. The report (in `full.train_filter` + `feature_report.train_filter`) always records the pre-cut `data_start`/`data_end` so the UI picker keeps full-span bounds (`fcst_eda_view._data_span`). All-rows-filtered → ValueError; < 2 seasonal cycles left → `train_filter_short` payload warning. A structural-break hint (`_break_hint`, from `aggregate.structural_breaks.break_dates`) suggests — never applies — a start date. Default is always full data; hard calendar filters (month/year subsetting) were deliberately NOT implemented — they break lag/seasonal feature continuity and the Fourier/calendar features already encode those patterns.
...
The catalog is `config/model_categories.yaml` — **12 models (2026-07-16)**: seasonal_naive (baseline reference), auto_theta, auto_ets, auto_arima (pmdarima), **mstl** (MSTL + Theta, `models_lib/statistical.py::MSTLForecaster` — added to the catalog 2026-07-16 for multi-seasonal series), prophet, croston/tsb (intermittent niche), lightgbm, xgboost, nhits (`library: torch` — pure-torch backend, see the statsforecast warning below), and **chronos** (`import_check: chronos` — Amazon's pretrained zero-shot transformer via `chronos-forecasting`, `models_lib/chronos_model.py`; variant in `settings.yaml → training.chronos_variant`, default chronos-bolt-small). `available` is derived at runtime via `importlib.util.find_spec`. Rule thresholds live in `config/settings.yaml → model_selection`.
...
statsforecast's compiled kernel `coreforecast` **segfaults (access violation 0xC0000005) under this venv's numpy 2.4.x** — verified repeatedly, no fixed release exists. **As of 2026-07-16 statsforecast is removed from `requirements.txt` entirely** (it only ever served the catalog's `find_spec` presence check and dragged in numba/llvmlite); the catalog cards that used to name it now carry their real runtime gates (`import_check: statsmodels` for auto_theta, `numpy` for the hand-rolled seasonal_naive/croston/tsb). **Never `import statsforecast` or add it back** — Stage 7's classical models are built on **statsmodels + pmdarima**, and the intermittent ones are hand-rolled; `models_lib/registry.py` is the real dispatch.
...
**Top-2 ensemble (2026-07-16, `training.ensemble_top2: true`):** after the model loop, `trainer._ensemble_top2` appends an `ensemble_top2` leaderboard row — the mean of the two best REQUESTED models' backtest/forecast display arrays (aligned on shared `ds`; different routes backtest different windows, e.g. chronos' single holdout vs classical CV — fewer than 3 shared points → no row), metrics on the same pooled MASE scale, conformal intervals from its own residuals. It's crownable (derived from requested models, so it respects the user's-selection-is-ultimate rule; `is_ensemble` marks it); when it wins, `best_model.pkl` is a dict of both components. Per-series runs also get averaged `ensemble_top2` rows in `series_forecasts.parquet` (mean where both components cover a `(unique_id, ds, kind)`). Not in the catalog/registry/estimator — it's a derived row, not a trainable model.

**CPU throttling (2026-07-16, `pipeline/resource_limits.py` + `settings.yaml → resources:`):** the job child runs at BELOW_NORMAL priority (`lower_priority()` in `jobs._entry`; ctypes with explicit `c_void_p` restype — default c_int truncates the pseudo-handle on 64-bit and silently fails) and every parallel library is capped to `resources.max_threads` (default cores−1): `apply()` exports OMP/OpenBLAS/MKL/NUMEXPR env caps and MUST run before the first numpy import — it's the first import side-effect of `api/jobs.py` and `api/tasks.py` (both re-imported in the spawn child) plus `api/main.py`/`frontend/app.py`; `ml_models._N_JOBS` replaces the old `n_jobs=-1`, and nhits/chronos call `torch.set_num_threads(thread_cap())` at their lazy torch entry points. `frontend/app.py` debug is env-driven (`APP_DEBUG=1`, set by run_frontend.bat only) — start_app.bat runs both servers without reload/debug watchers.

**Direct multi-horizon ML backend** (`ml_models.py`, rewritten 2026-07 from the manual milk pipeline's strategies — the recursive rollout is gone): per-series **seasonal-index deseasonalization** (phase mean / overall mean, clipped 0.2–5.0 — trees can't extrapolate multiplicative seasonality; flat-index fallback for negative/zero-level series), origin features from `feature_builder.build_supervised` expanded × horizon steps (`horizon`/`target_phase`/`target_sindex` features), **quantile objectives** (P50 = the point model; interval alphas fitted lazily by `predict_quantiles`, non-crossing enforced by sorting), **early stopping on the last ~20% of origins then refit at the found tree count on ALL rows**, base params derived from the manual milk-pipeline experiments (n_estimators 800 cap, lr 0.025, leaves 63…). `run_global` adds `series_id` + per-series level/scale scalars + `share_lag1` (share of panel total at t−1) and a stratified `(series, horizon)` row cap (`training.max_train_rows`). Optuna (`tune`) searches around the base params on the same direct dataset; trials come from the **accuracy profile**.
...
Adaptive CV (`_cv_plan`): `n_windows` sized so `n_windows·h ≤ len − min_train`, capped at `training.cv_windows` (shrinks to `cv_windows_large_panel` on big panels); too short → single holdout → else fit-only. **Cost guardrails (rewritten 2026-07-14 — wall-clock, self-calibrating):** `estimate_cost` predicts SECONDS per model from work-based priors (global ML from capped direct-dataset rows + per-series loop overhead; classical from `(CV fits+1) × per-model fit cost × capped fan-out`; nhits from profile epochs × frame rows; chronos from batched inference size), each multiplied by a per-`(model, route)` calibration factor learned from past runs' `train_seconds` (`record_timings` → `models/timing_calibration.json`, median of last 6, clipped [0.2, 5] — machine-specific and self-correcting). Tier = estimated minutes vs `training.tier_minutes {moderate: 2, heavy: 15}` (the old `*_tier_units` keys are gone). `estimate_for_run(run_id, models, profile)` is the one authoritative entry point (reads payload/selections/msel from disk) used by BOTH the `/train` heavy gate (400 + estimate dict when unconfirmed) and `GET /runs/{id}/train-estimate` (the Training tab's live banner — the frontend no longer mirrors the math; unreachable API → an honest "estimate unavailable" banner). **The per-series fan-out cap applies to ANY per-series-routed model regardless of policy** (`per_series_cap: 200`, profile-independent) — previously it only applied under `per_series_local`, so with a `global` policy a selected prophet fanned out over ALL 1,782 Favorita series (hours of Stan fits) while the old unit estimate charged it 1 unit and said "fast". Also `max_models_per_run: 5`. Thresholds in `config/settings.yaml → feature_engineering` / `→ training`. Reference (balanced profile, direct backend): milk → **xgboost 0.795** best / lightgbm 0.899 (old recursive LGBM was 0.9365) with native quantile intervals; vet → auto_theta 0.5998 best (lightgbm 0.655). Milk 5-model run (2026-07-09, 74s): **chronos 0.861** best-of-requested / lightgbm 0.899 / baseline 0.949 / prophet 1.25 / auto_theta 1.33 / auto_arima 1.39 — the zero-shot model is a genuinely strong cheap candidate.

### Frontend — 3-section UI (Dash 4.3.0, restructured 2026-07-14)

The UI is a single-page Dash app with a **static 3-section tab bar** (`dcc.Tabs` in `frontend/layout.py`): **Setup** (`tab-setup`: ingestion form + auto-run switch + past runs · dataset profile `data-tab-results` + forecast-intent confirmation `intent-form-body`), **Pipeline** (`tab-pipeline`: live stage rail `stage-rail` + log panel `pipeline-log` · four static `ui.stage_card` expanders wrapping the per-stage result bodies), **Results** (`tab-results`: training leaderboard/chart/insights via `training_view.results_block` + LLM report + downloads). All six pre-refactor body div ids SURVIVE re-parented (`data-tab-results`, `clean-tab-body` [cleaning+validation only — intent moved to Setup], `fcst-eda-tab-body`, `model-tab-body`, `training-tab-body` [controls only: picker/estimate/Train button — `training_view.render_training_controls`], `results-tab-body`), so every store-driven render callback kept working. The clientside pane switcher has a **stale-tab fallback** (persisted pre-refactor values like `"tab-data"` → `tab-setup`) — required, `dcc.Tabs(persistence=True)` restores old sessions' values. All state lives in `dcc.Store(id="results-store", storage_type="session")`.

**Auto-run ("Run everything", 2026-07-14):** `switch-auto-run` (static, in the Data Source card, default ON) + `dcc.Store(id="auto-run-store", session)` state machine (`phase: await_intent → train_pending → done | failed | running_train`). `trigger_run` (ingest→profile→intent) sets `await_intent`; the Setup intent form shows a pause banner; `confirm_and_run` (clean→validate→EDA→model-select) advances to `train_pending` **only when that confirm IS the pending pause** (`auto_continue` = switch on AND phase == `await_intent` AND matching run_id, 2026-07-15) — any later re-confirm (fixing the series key/target) is always manual, stops after model selection, and CLEARS the auto phase, so training never fires as a side effect of iterating on the setup; `auto_run_train` (Input = auto-run-store, guards ARE the mount-fire guard — phase+run_id+`_stage5` checks, plus a **refresh-safety check**: if `GET /progress` shows train running with `job_active`, do NOT re-POST /train — it would preempt and kill the in-flight job) trains the Stage-5 top-2 (`training_view.default_selection`) on the default profile with `confirm_heavy=True`, then auto-generates the Stage 8 report (failure non-fatal → manual button remains), lands on Results. `_post_train()` is the shared POST-/train helper (manual `run_training` + auto). Every reset path (`new_dataset`, `load_past_run`, `trigger_run`) clears/overwrites auto-run-store so a stale phase can't fire on the wrong run.

**Setup declutter (2026-07-15):** the Setup right column is now three `ui.stage_card` expanders — **Dataset** (shape card + schema + numeric stats; `summary_extra="N rows × M cols"` in the summary row) and **Pre-clean EDA** (missing/outliers/flags; when Stage 2 hasn't run, the body is `_eda_retry_prompt()` and the card renders open), both collapsed by default, plus **Forecast Setup** (`render_intent_form` wraps the intent banner+form in `stage_card("intent", …, open=True)` — same heading chrome as its siblings; the form's inner title is gone, the Re-detect button sits beside the detection note). `stage_card` gained the optional `summary_extra` right-aligned muted text. The dataset header's "New Run" button (`btn-clear-results`) and its `clear_results` callback are REMOVED — the reset is the left column's "Load Different Dataset" (`btn-new-run`). The Forecast EDA tab was trimmed the same day: ACF/PACF charts and the STL/FFT seasonality tables are gone entirely (`_acf_fig`/`_seasonality_section` deleted); the decomposition chart, exogenous tables and panel top-series deep-dive (overlay chart + table) are demoted into `collapse_section` expanders — visible content is just warnings, the 4 verdict tiles, the setup recap/re-run form and the analysis-series chart.

**Live progress rail + logs:** `pipeline/progress.py` writes `runs/{id}/progress.json` atomically (tmp+`os.replace`; UTF-8; ~200-event log ring; timestamps are LOCAL-aware ISO, 2026-07-15 — the log panel slices wall-clock time straight from the string and UTC was hours off; `stage_finish` subtracts aware datetimes so old UTC entries still compute) from inside the job child; `ui.log_panel` renders newest-FIRST (each 1.5s poll re-render resets scroll to top, so the top must hold the current activity) — **`stage_start` sweeps stale "running" entries (2026-07-15):** a preempted job's stage stays "running" forever (its `stage_finish` never runs), and while the NEXT job is active the `/progress` job-alive reconciliation can't tell it's dead — the rail showed e.g. intent AND clean running at once. Since the job manager is single-slot, `stage_start` now marks any other "running" stage "cancelled" (except `report`, exempt both directions — it runs in-process and legitimately overlaps jobs) — `api/tasks.py` wrappers bracket every stage (`_staged`), `trainer.run`'s model loop emits per-model events (running/done/failed + ETA from the cost estimate + MASE + seconds), `forecasting_eda.run` emits sub-steps, the `/train` route seeds the ETA pre-dispatch (`stage_start` preserves an existing `eta_seconds`), `/report` brackets in-process, `/clean` + `/forecast-eda` call `reset_downstream`. `GET /runs/{id}/progress` = file + `job_active` (`jobs.active_track_id()`) so a stranded `running` (killed job) renders as `cancelled` — except the `report` stage (in-process, never owns the job slot). Frontend: `dcc.Interval(id="progress-interval", 1.5s, disabled=True)` is enabled via the `running=` spec of every long chain callback (string ids only); `render_progress` (Interval + store) merges store-derived done markers → progress.json → cancelled override → auto-phase `queued` markers, renders `ui.stage_rail`/`ui.log_panel` + the 4 `stage-badge-*` spans, and offers a **Resync results** button when the file says train done but the store lacks `_stage7` (refresh mid-train; `resync_run` → `/summary`). The rail/log divs are deliberately NOT in the big `dcc.Loading` target set (polls must not flash the spinner). The Interval-poll-during-blocking-callback pattern works because the Flask dev server is threaded.

**Theme tokens (2026-07-14):** `3_custom.css` defines `--ink-strong/--ink/--ink-muted/--ink-faint/--ink-ghost/--surface-card/--surface-inset/--accent/--accent-soft/--ok/--warn/--bad` on `:root, body`, overridden in `body.light-mode` (`--ink-muted: #475569` on white — the light-mode contrast fix; light accent stays green). ~105 inline grey hex styles across the views were swept to `var(--ink-*)` (inline `var()` resolves against the cascade, so inline styles theme-switch). Utility classes: `.ink-muted/.ink-faint/.ink-ghost/.section-header/.stat-label/.stat-value/.hint-text` + rail/log/stage-card CSS. **Plotly figure paint (`_INK`/`_CHART_BG`/`_GRID`/`_C_*` constants) must stay real hex** — canvas can't resolve CSS vars; never sweep those constants or `dict(color=...)` kwargs in figure code.

**All section panes stay mounted; tab switching is clientside-only.** A `clientside_callback` on `stage-tabs.value` toggles `style.display` on the three pane divs. Server callbacks re-render only the data-dependent bodies and only on `results-store` changes. Do **not** go back to rebuilding pane content on `stage-tabs.value` (the old `render_tab` pattern): constant unmount/remount of components triggered both renderer bugs below on every switch. Keeping the form mounted also preserves typed DB credentials across tab switches.
...
- Every button callback guards `if not n_clicks: return no_update, ...`. Before these guards, the reset callbacks (`new_dataset`, and the since-removed `clear_results`) fired when their buttons mounted → wiped the store ("switching back to the Data tab resets the run") and their `_cancel_running()` call killed in-flight jobs (bogus 409s on Stage 2).
...
- `suppress_callback_exceptions=True` — still required for the components rendered dynamically inside the pane bodies (`btn-confirm-run`, the `dropdown-intent-*` family, `btn-fcst-rerun`, `btn-model-rerun`, `switch-model-llm`, `btn-run-eda`)
...
- **dcc.Loading spinner clipping (2026-07-15):** the spinner overlay is `position:absolute; top:0; height:100%` of the wrapper, flex-centered — inside a near-zero-height wrapper (the empty `cleaning-status` div) the 40px circle straddled the top edge, half-hidden under the tab bar. Fixes: `overlay_style={"minHeight": "64px"}` on the cleaning-status Loading (applies to the hidden-children div ONLY while loading, so no permanent gap), and CSS `.dash-sk-circle { margin: 2px auto }` (ships with `1rem auto` — 72px total never fit the ~38px button wrappers).
