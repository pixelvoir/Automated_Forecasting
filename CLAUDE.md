...
"last interrupted prompt: few fixes: in the setup page, remove the new runbutton, make the dataset and pre clean eda into expandable tabs like you did the cleaning, forecast eda and model selection in the pipeline tab. change the font and icon color of forecast setup to match the other headings, dataset and pre clean eda. trim the forecast EDA tab, no viewer would view that much data, remove some of the graphs and tables. and make sure after model training it moves to results tab, if it doesnt aleady.
"
